    ...
    connection.onCodeAction((params) => {
        ...
        for (const diagnostic of params.context.diagnostics) {
            if (diagnostic.message.includes('does not exist on')) {
                const suggestions = getSuggestionsForMissingMember(diagnostic, document.getText(), documentationService);
 ...
 */
function createCodeActionsForDiagnostic(diagnostic, _params, uri) {
    const actions = [];
 ...
 */
function createAddSemicolonAction(diagnostic, uri) {
 ...
 */
function createCloseStringAction(diagnostic, uri) {
    const line = diagnostic.range.end.line;
    const character = diagnostic.range.end.character;
    const edit = {
        changes: {
 ...
 */
function createRemoveBraceAction(diagnostic, uri) {
    const edit = {
        changes: {
            [uri]: [{
 ...
 */
function createRemoveNullAction(diagnostic, uri) {
    return {
 ...
 */
function getSuggestionsForMissingMember(diagnostic, _documentText, documentationService) {
    const actions = [];
    const message = diagnostic.message;
    ...
    for (const suggestion of suggestions.slice(0, 3)) {
        actions.push(createRenameAction(diagnostic, wrongName, suggestion, isMethod ? 'method' : 'property'));
    }
    return actions;
}
/**
 ...
 */
function createRenameAction(diagnostic, _wrongName, correctName, _type) {
    return {
...
function findSimilarNames(target, candidates) {
    ...
    const scored = candidates.map(candidate => {
        ...
        if (candidateLower.startsWith(targetLower.substring(0, 3)))
            score += 50;
        // Contains target
