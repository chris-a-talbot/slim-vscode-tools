...
exports.validateInitializationRules = validateInitializationRules;
 ...
 */
function validateInitializationRules(_text, lines) {
    const diagnostics = [];
    const tracking = {
        modelTypeLine: null,
 ...
 */
function trackInitializationCalls(lines, tracking) {
 ...
 */
function validateInitializationOrder(tracking, lines) {
    const diagnostics = [];
    ...
    if (tracking.modelTypeLine !== null && tracking.slimOptionsLine !== null) {
        if (tracking.slimOptionsLine < tracking.modelTypeLine) {
            ...
            diagnostics.push((0, diagnostics_1.createDiagnostic)(vscode_languageserver_1.DiagnosticSeverity.Error, tracking.slimOptionsLine, col, col + 'initializeSLiMOptions'.length, 'initializeSLiMOptions() must be called after initializeSLiMModelType()'));
        }
    }
    ...
    if (tracking.slimOptionsLine !== null && tracking.chromosomeLine !== null) {
        if (tracking.chromosomeLine < tracking.slimOptionsLine) {
            ...
            diagnostics.push((0, diagnostics_1.createDiagnostic)(vscode_languageserver_1.DiagnosticSeverity.Error, tracking.chromosomeLine, col, col + 'initializeChromosome'.length, 'initializeChromosome() must be called after initializeSLiMOptions()'));
        }
    }
    return diagnostics;
}
 ...
 */
function validateRequiredGeneticComponents(tracking, lines) {
    const diagnostics = [];
    ...
    if (!tracking.mutationTypeCalled) {
        ...
        diagnostics.push((0, diagnostics_1.createDiagnostic)(vscode_languageserver_1.DiagnosticSeverity.Warning, initLine, col, col + 'initialize'.length, 'Missing initializeMutationType() - required for genetics models'));
    }
    if (!tracking.genomicElementTypeCalled) {
        const line = lines[initLine];
        const col = line.indexOf('initialize');
        diagnostics.push((0, diagnostics_1.createDiagnostic)(vscode_languageserver_1.DiagnosticSeverity.Warning, initLine, col, col + 'initialize'.length, 'Missing initializeGenomicElementType() - required for genetics models'));
    }
    if (!tracking.genomicElementCalled) {
        const line = lines[initLine];
        const col = line.indexOf('initialize');
        diagnostics.push((0, diagnostics_1.createDiagnostic)(vscode_languageserver_1.DiagnosticSeverity.Warning, initLine, col, col + 'initialize'.length, 'Missing initializeGenomicElement() - required for genetics models'));
    }
    if (!tracking.mutationRateCalled) {
        const line = lines[initLine];
        const col = line.indexOf('initialize');
        diagnostics.push((0, diagnostics_1.createDiagnostic)(vscode_languageserver_1.DiagnosticSeverity.Warning, initLine, col, col + 'initialize'.length, 'Missing initializeMutationRate() - required for genetics models (unless nucleotide-based)'));
    }
    if (!tracking.recombinationRateCalled) {
        const line = lines[initLine];
        const col = line.indexOf('initialize');
        diagnostics.push((0, diagnostics_1.createDiagnostic)(vscode_languageserver_1.DiagnosticSeverity.Warning, initLine, col, col + 'initialize'.length, 'Missing initializeRecombinationRate() - required for genetics models'));
    }
    return diagnostics;
}
/**
 ...
 */
function validateNonWFRequirements(tracking, lines) {
    const diagnostics = [];
    ...
    if (tracking.hasReproductionCallback && tracking.modelType === 'WF') {
        if (tracking.reproductionCallbackLines.length > 0) {
            ...
            diagnostics.push((0, diagnostics_1.createDiagnostic)(vscode_languageserver_1.DiagnosticSeverity.Error, tracking.reproductionCallbackLines[0], col, col + 'reproduction'.length, 'reproduction() callback requires nonWF model (call initializeSLiMModelType("nonWF"))'));
        }
    }
    return diagnostics;
}
 ...
 */
function validateInitializeScope(lines, tracking) {
    const diagnostics = [];
    if (!tracking.hasInitializeCallback) {
        return diagnostics;
    }
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (/\bsim\b/.test(line) && !/^\s*\/\//.test(line)) {
            ...
            diagnostics.push((0, diagnostics_1.createDiagnostic)(vscode_languageserver_1.DiagnosticSeverity.Error, i, col, col + 3, 'The "sim" symbol is not defined within initialize() callbacks'));
        }
        // Check for community usage (but not in comments)
        if (/\bcommunity\b/.test(line) && !/^\s*\/\//.test(line)) {
            const col = line.indexOf('community');
            diagnostics.push((0, diagnostics_1.createDiagnostic)(vscode_languageserver_1.DiagnosticSeverity.Error, i, col, col + 9, 'The "community" symbol is not defined within initialize() callbacks'));
        }
    }
    return diagnostics;
}
//# sourceMappingURL=initialization-rules.js.map