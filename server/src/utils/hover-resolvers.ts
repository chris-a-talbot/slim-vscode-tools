...
import { CLASS_NAMES, EIDOS_EVENT_NAMES } from '../config/config';
...
export function getOperatorHover(
    word: string,
    operatorsData: Record<string, OperatorInfo>
): Hover | null {
    for (const [operatorName, operatorInfo] of Object.entries(operatorsData)) {
        if (
            operatorInfo.signature.includes(word) ||
            operatorName.toLowerCase().includes(word.toLowerCase())
        ) {
            return createHoverResponse(createOperatorMarkdown(operatorName, operatorInfo));
        }
    }
