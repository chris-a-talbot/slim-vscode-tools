...
import { IDENTIFIER_PATTERNS, FORMATTING_PATTERNS } from '../config/config';
...
export function getOperatorAtPosition(text: string, position: Position): string | null {
    ...
    for (const op of FORMATTING_PATTERNS.TWO_CHAR_OPS) {
    ...
    if (char && FORMATTING_PATTERNS.SINGLE_CHAR_OPS.includes(char as any)) {
