...
function getWordAtPosition(text: string, position: Position): WordInfo | null {
    ...
    if (character > 0 && line[character - 1] === '"') {
        // We're right after opening quote
        start = character;
        end = character;
    ...
    // Check if cursor is inside quoted string
    let quoteStart = -1;
...
function findDefinition(
    ...
    if (callbackName) {
        ...
        if (word in pseudoParams) {
            // These are implicit - find the callback declaration line by searching backward
            // Look for the actual callback pattern (e.g., "reproduction("), not just the word
            ...
            for (let i = cursorPosition.line; i >= 0; i--) {
                // Skip comments and match actual callback declaration
    ...
    // These are typically defined with quoted identifiers
    ...
    if (trackingState.definedSubpopulations?.has(word)) {
        // Search for the actual addSubpop call with this identifier
        // Pattern supports: sim.addSubpop("p1"), species.addSubpop("p1"), etc.
        const subpopPattern = `addSubpop("${word}"`;
        const defLine = findFirstOccurrence(lines, subpopPattern, cursorPosition.line);
        if (defLine !== -1) {
            const startChar = lines[defLine].indexOf(`"${word}"`);
            if (startChar !== -1) {
                return Location.create(uri, Range.create(
    ...
    if (trackingState.definedSpecies?.has(word)) {
        // Look for species declaration: "species word initialize()"
    ...
    // Priority 5: Check for variable declarations (first assignment only)
    // Look for the FIRST assignment before cursor position
...
/**
 * Find first occurrence of a pattern, searching backwards from cursor position
 * and then forward if not found
 */
