...
import { CONTROL_FLOW_KEYWORDS } from '../config/constants';
 ...
 */
export function tokenizeSLiM(text: string): Token[] {
    const tokens: Token[] = [];
    let i = 0;
    ...
    while (i < len) {
        ...
        if (TEXT_PROCESSING_PATTERNS.WHITESPACE.test(text[i])) {
            i++;
            continue;
        }
        ...
        // Numbers
        if (TEXT_PROCESSING_PATTERNS.DIGIT.test(text[i]) || (text[i] === '.' && TEXT_PROCESSING_PATTERNS.DIGIT.test(text[i + 1]))) {
            const start = i;
            while (i < len && (TEXT_PROCESSING_PATTERNS.NUMBER.test(text[i]) || (text[i] === '.' && TEXT_PROCESSING_PATTERNS.DIGIT.test(text[i + 1])))) {
                i++;
            }
        ...
        if (TEXT_PROCESSING_PATTERNS.OPERATOR_PUNCTUATION.test(text[i])) {
        ...
        if (TEXT_PROCESSING_PATTERNS.IDENTIFIER_START.test(text[i])) {
            const start = i;
            while (i < len && TEXT_PROCESSING_PATTERNS.IDENTIFIER_CHAR.test(text[i])) {
                i++;
            }
            const value = text.substring(start, i);
            const isKeyword = CONTROL_FLOW_KEYWORDS.includes(value);
            tokens.push({ 
