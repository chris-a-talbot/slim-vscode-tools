...
export class DocumentationService {
    ...
    public loadDocumentation(): void {
        try {
            ...
            log(`Loaded eidos functions: ${Object.keys(this.functionsData).length} total functions`);
    ...
    private loadFunctionData(
        ...
        const data = this.loadJsonFile<Record<string, Record<string, { signatures: string[]; description: string }>>>(filePath);
        if (!data) return;
    ...
    private loadCallbackData(
        filePath: string,
        target: Record<string, CallbackInfo>
    ): void {
        const data = this.loadJsonFile<Record<string, { signature: string; description: string }>>(filePath);
        if (!data) return;
    ...
    private loadOperatorData(
        filePath: string,
        target: Record<string, OperatorInfo>
    ): void {
        const data = this.loadJsonFile<Record<string, { signature?: string; description: string }>>(filePath);
        if (!data) return;
