...
import { countBracesIgnoringStringsAndComments } from '../utils/text-processing';
...
class DocumentValidator {
    ...
    public validate(): Diagnostic[] {
        ...
        this.diagnostics.push(
            ...validateDefinitions(this.text, this.lines),
            ...validateScriptStructure(this.text, this.lines),
            ...validateUndefinedReferences(this.text, this.lines)
        );
    ...
    private validateSemanticAspects(
        ...
        if (instanceDefinitions && classesData) {
            this.diagnostics.push(...validateMethodOrPropertyCall(line, lineIndex, instanceDefinitions, classesData));
        }
        
        // Validate function calls if callbacks data is available
        if (callbacksData) {
            this.diagnostics.push(...validateFunctionCalls(line, lineIndex, functionsData, callbacksData));
        }
        
        // Validate null assignments if all required data is available
        if (instanceDefinitions) {
            this.diagnostics.push(...validateNullAssignments(line, lineIndex, functionsData, classesData, instanceDefinitions));
        }
    }
