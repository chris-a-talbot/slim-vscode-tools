...
export function checkDuplicateDefinition(
    ...
    if (match && match.index !== undefined) {
        ...
        if (seenMap.has(id)) {
            ...
            const endChar = startOffset > 0 ? startChar + id.length : match.index + match[0].length;
            return createDiagnostic(
                DiagnosticSeverity.Error,
                lineIndex,
                startChar,
                endChar,
                `${typeName} ${id} already defined (first defined at line ${seenMap.get(id)! + 1})`
            );
        } else {
...
export function validateDefinitions(_text: string, lines: string[]): Diagnostic[] {
    ...
    lines.forEach((line, lineIndex) => {
        ...
        if ((match = line.match(defineConstantRegex)) !== null && match.index !== undefined) {
            ...
            if (RESERVED_IDENTIFIERS.has(constName)) {
                diagnostics.push(createDiagnostic(
                    ...
                    constStartChar,
                    constEndChar,
                    ERROR_MESSAGES.RESERVED_IDENTIFIER(constName, 'a global constant')
                ));
                ...
                diagnostics.push(createDiagnostic(
                    ...
                    constStartChar,
                    constEndChar,
                    ERROR_MESSAGES.DUPLICATE_DEFINITION(TYPE_NAMES.CONSTANT, constName, seenConstants.get(constName)! + 1)
                ));
            } else {
        ...
        if ((match = line.match(subpopRegex)) !== null || (match = line.match(subpopSplitRegex)) !== null) {
            if (match && match.index !== undefined) {
                const subpopName = match[1];
                if (seenSubpopulations.has(subpopName)) {
                    // Find the position of the subpopulation name in the match
                    const nameStartChar = match.index + match[0].indexOf(`"${subpopName}"`);
                    const nameEndChar = nameStartChar + subpopName.length + 2;
                    diagnostics.push(createDiagnostic(
                        DiagnosticSeverity.Error,
                        lineIndex,
                        nameStartChar + 1,
                        nameEndChar - 1,
                        ERROR_MESSAGES.DUPLICATE_DEFINITION(TYPE_NAMES.SUBPOPULATION, subpopName, seenSubpopulations.get(subpopName)! + 1)
                    ));
                } else {
        ...
        if ((match = line.match(speciesRegex)) !== null && match.index !== undefined) {
            ...
            if (RESERVED_IDENTIFIERS.has(speciesName)) {
                ...
                diagnostics.push(createDiagnostic(
                    ...
                    speciesStartChar,
                    speciesEndChar,
                    ERROR_MESSAGES.DUPLICATE_DEFINITION(TYPE_NAMES.SPECIES, speciesName, seenSpecies.get(speciesName)! + 1)
                ));
            } else {
