...
import { ERROR_MESSAGES, EVENT_PATTERNS } from '../config/config';
...
function validateBraceBalance(
    ...
    // Check for unexpected closing braces (but not in SLiM event blocks)
...
function validateUnclosedString(
    ...
    // String is still open at end of document
...
export function shouldHaveSemicolon(
    ...
    const isDefinitelySafe =
        ...
        cleaned.endsWith('}') ||
        netParens > 0 ||
        /^\s*(if|else|while|for|switch|case|default)\b.*\)?\s*{?\s*$/.test(cleaned) ||
        /^(initialize|early|late|fitness)\s*\([^)]*\)\s*{?\s*$/.test(cleaned) ||
        /^\s*(s\d+\s+)?\d+\s+(early|late|reproduction|fitness)\s*\(\)\s*$/.test(cleaned) ||
        /^\s*\/[\/\*]/.test(line) ||
        /^\s*\*/.test(line) ||
        /^\s*$/.test(line);
