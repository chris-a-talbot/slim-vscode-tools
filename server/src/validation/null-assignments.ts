...
import { isNullableType, extractParameterTypes, resolveClassName, ParameterInfo } from '../utils/type-manager';
...
function createNullAssignmentDiagnostic(
    ...
    const paramName = param.name || `parameter ${paramIndex + 1}`;
...
function parseFunctionArguments(line: string, parenStart: number, parenEnd: number): ArgumentInfo[] {
    const argString = line.substring(parenStart + 1, parenEnd);
    const args: ArgumentInfo[] = [];
    let currentArg = '';
    let argStartPos = parenStart + 1;
    let argDepth = 0;
    let inString = false;
    ...
    for (let i = 0; i < argString.length; i++) {
        ...
        const fullLinePos = parenStart + 1 + i;
...
export function validateNullAssignments(
    ...
    while ((match = functionCallRegex.exec(line)) !== null) {
        ...
        const args = parseFunctionArguments(line, parenStart, argEnd - 1);
        ...
        if (methodMatch) {
            const instanceName = methodMatch[1];
